...
RSpec.describe Space::Flight::ShipGateway do
  ...
  context 'when updating a ship record' do
    let(:ship_record) { instance_double('Ship', id: 1, crew_ids: [], crew: [], dock: nil, location: nil, location_id: nil, update: true) }
    let(:ship_repository) { class_double('Ship', find: ship_record) }

    subject { described_class.new(ship_repository: ship_repository).update(1, location: nil) }
    ...
    it 'should call update with attrs' do
      subject
      expect(ship_record).to have_received(:update).with(location: nil)
    end
  end
